...
#' @param habitat_data [FUTURE FEATURE] *string* name of a csv file containing habitat data. 
#'   Data should be in format generated by 'Comprehensive Site Data' query and stored in 
#'   the /data directory of the ltm report template. 
#'
...
## habitat_data -----------------------------------------
if(stringr::str_sub(habitat, -4) != \".csv\") {
habitat <- paste(habitat, \".csv\", sep=\"\")
}

habitat <- 
  paste(\"data/\",habitat,sep=\"\") %>%
  read.csv() %>%
  data.frame()
...
fhc_nocode <- 
  fhc_sum %>%
  filter(year == max(fhc_sum$year), FishHealthCode1 == \"\") %>% 
  pull(pct) %>%
  round(2)    
...
fhc_p <- fhc_sum %>% filter(year == max(fhc_sum$year), FishHealthCode1 == \"P\") %>% pull(n)
...
pv_yr <- lubridate::year(end_date)-1
